...
from src.mooseClient import discordClient
...
def run_discord_bot():
    ...
    async def on_ready():
        ...
        logger.info(f'{discordClient.user} is ready!')
    ...
    @discordClient.tree.command(name="stream", description="Play YT URL")
    ...
    @discordClient.tree.command(name="chat", description="Have a chat with Moose")
    async def chat(interaction: discord.Interaction, *, message: str):
        if discordClient.is_replying_all == "True":
            await interaction.response.defer(ephemeral=False)
            await interaction.followup.send("> **WARN: You already are on replyAll mode. If you want to use the Slash Command, switch to normal mode by using `/replyall` again**")
        ...
        logger.info(f"\x1b[31m{username}\x1b[0m : /chat [{message}] in ({discordClient.current_channel})")
    ...
    async def private(interaction: discord.Interaction):
        ...
        if not discordClient.isPrivate:
            ...
            await interaction.followup.send(
                "> **WARN: You already are on private mode. If you want to switch to public mode, use `/public`**")
    ...
    async def public(interaction: discord.Interaction):
        ...
        if discordClient.isPrivate:
            ...
            await interaction.followup.send(
                "> **WARN: You already are on public mode. If you want to switch to private mode, use `/private`**")
    ...
    async def chat_model(interaction: discord.Interaction, model: app_commands.Choice[str]):
        ...
        try:
            if model.value == "gemini":
                ...
                discordClient.reset_conversation_history()
                discordClient.chatBot = Client(
                    provider=RetryProvider([FreeGpt, ChatgptNext, AItianhuSpace], shuffle=False))
                discordClient.chatModel = model.value
